...
class ResearchAgent {
    ...
    setupEventListeners() {
        ...
        // Copy Report
        document.getElementById('copy-btn')?.addEventListener('click', () => this.copyReport());
...
${'='.repeat(this.currentReport.title.length)}
...
Sources:
${this.currentReport.sources.map((source, i) => `${i + 1}. ${source.title} - ${source.url}`).join('\n')}

---
Generated by ResearchGO
Date: ${new Date(this.currentReport.created_at).toLocaleDateString()}
            `.trim();
            ...
            this.showError('Failed to copy report. Please try again.');
        }
    }
